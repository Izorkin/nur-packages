...
rec {
  ...
  mariadb             = pkgs.callPackage  ./pkgs/servers/mariadb { openssl = pkgs.libressl; asio = pkgs.asio_1_10; jemalloc = pkgs.jemalloc.override ({ disableInitExecTls = true; }); inherit (pkgs.darwin) cctools; inherit (pkgs.darwin.apple_sdk.frameworks) CoreServices; };
